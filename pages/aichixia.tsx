...
e.preventDefault();
if (!loading && cooldownRemaining === 0) sendMessage();
}
};

const headerScanDisabled = loading || cooldownRemaining > 0;
const footerSendDisabled = loading || cooldownRemaining > 0 || !session;

return (
<> <Head> <title>Aichixia | AI Assistant</title> <meta
       name="description"
       content="Aichixia is your AI assistant for anime, manga, manhwa, and light novels. Chat or identify anime from screenshots!"
     /> <meta property="og:title" content="Aichixia AI Assistant" /> <meta
       property="og:description"
       content="Chat or identify anime instantly with Aichixia."
     /> <meta property="og:image" content="/aichixia.png" /> <meta property="og:type" content="website" /> <link rel="icon" href="/favicon.ico" /> </Head>
  ...
  <main className="flex flex-col items-center min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 text-white relative overflow-hidden">
      ...
      <header className="p-4 sm:p-5 border-b border-blue-500/20 bg-slate-900/40 backdrop-blur-2xl rounded-b-2xl shadow-2xl flex items-center justify-between sticky top-0 z-20 mt-2 sm:mt-4">
        ...
        <div className="relative">
          <button
            ...
            className={`relative group p-3 sm:p-3.5 rounded-2xl transition-all duration-300 ${headerScanDisabled ? "opacity-50 pointer-events-none scale-100 bg-slate-800/30 border border-blue-500/10" : "bg-gradient-to-br from-blue-500 via-blue-600 to-cyan-500 hover:shadow-2xl hover:shadow-blue-500/40 hover:scale-105 active:scale-95"}`}
            aria-disabled={headerScanDisabled}
          >
          ...
          {cooldownRemaining > 0 && (
            <div className="absolute -right-2 -bottom-3 bg-slate-800/80 text-xs px-2 py-1 rounded-full border border-blue-500/20">
              {cooldownRemaining}s
            </div>
          )}
        </div>
      ...
      <section className="flex-1 overflow-y-auto py-6 space-y-5 scrollbar-thin scrollbar-thumb-blue-500/30 scrollbar-track-transparent px-1">
        {messages.map((msg, i) => (
          ...
          >
            {msg.type === "text" && (
              <div
                className={`px-5 py-4 rounded-3xl max-w-[85%] sm:max-w-[75%] text-sm sm:text-base shadow-xl backdrop-blur-xl ${msg.role === "user" ? "bg-gradient-to-br from-blue-500 via-blue-600 to-cyan-500 text-white shadow-blue-500/30" : "bg-slate-800/60 border border-blue-500/20 text-slate-100 shadow-slate-900/50"}`}
              >
                <ReactMarkdown 
        ...
        {!session ? (
          ...
          <div className="flex gap-2 sm:gap-3 items-center">
            ...
            <div className="relative">
              <button
                onClick={sendMessage}
                disabled={footerSendDisabled}
                className={`relative group p-3 sm:p-4 rounded-2xl transition-all duration-300 ${footerSendDisabled ? "opacity-50 pointer-events-none bg-slate-800/30 border border-blue-500/10" : "bg-gradient-to-br from-blue-500 via-blue-600 to-cyan-500 hover:shadow-2xl hover:shadow-blue-500/40 hover:scale-105 active:scale-95"}`}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
                {loading ? (
    ...
    <AnimatePresence>
      {scanOpen && (
        ...
        >
          <motion.div
            className="bg-gradient-to-br from-slate-900/90 to-slate-800/80 rounded-3xl p-6 sm:p-8 w-full max-w-lg text-center shadow-2xl border border-blue-500/20 relative backdrop-blur-3xl"
          ...
          >
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-14 h-14 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-3xl flex items-center justify-center shadow-2xl shadow-blue-500/40">
              <LuScanLine className="text-2xl text-white" />
            </div>

            <h2 className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-300 mb-2 mt-3">
              Upload Screenshot
            </h2>
            <p className="text-blue-300/70 text-sm sm:text-base mb-6 font-light">
            ...
            {!session ? (
              <Link
                href="/auth/login"
                className="inline-flex items-center justify-center gap-3 px-6 py-3 bg-gradient-to-r from-blue-500 via-blue-600 to-cyan-500 hover:shadow-2xl hover:shadow-blue-500/40 text-white rounded-2xl font-bold transition-all duration-300 hover:scale-105 active:scale-95"
              >
              ...
              <div className="flex flex-col items-center gap-4">
                <label className="w-full cursor-pointer inline-flex items-center justify-center gap-3 px-6 py-3 bg-slate-800/60 hover:bg-slate-800/70 rounded-2xl text-blue-200 font-semibold transition-all duration-300 hover:scale-102 active:scale-95 border border-blue-500/20">
                  <LuScanLine className="text-xl" />
                ...
                {pendingImage && (
                  <motion.div 
                    ...
                    animate={{ scale: 1, opacity: 1 }}
                    className="mt-2 relative w-56 sm:w-64"
                  >
                    <div className="relative w-full h-56 sm:h-64 border-2 border-blue-400/30 rounded-2xl overflow-hidden shadow-2xl shadow-blue-500/20">
                      <Image
                        src={pendingImage}
                        alt="preview"
                        fill
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 bg-red-500/85 backdrop-blur-xl rounded-full p-2 hover:bg-red-600 transition-all hover:scale-110 active:scale-95 shadow-lg"
                      >
                ...
                <div className="mt-4 flex flex-col sm:flex-row gap-3 justify-center w-full">
                  <button
                    onClick={() => setScanOpen(false)}
                    className="px-5 py-3 bg-slate-700/50 hover:bg-slate-700/70 rounded-2xl text-blue-200 transition-all hover:scale-105 active:scale-95 font-semibold backdrop-blur-xl border border-blue-500/20 w-full sm:w-auto"
                  >
                    Cancel
                  </button>
                 ...
                 {session && pendingImage && (
                    <button
                      onClick={() => {
                        if (cooldownRemaining === 0 && !loading) {
                          setScanOpen(false);
                          startCooldown(30);
                          sendMessage();
                        }
                      ...
                      disabled={loading || cooldownRemaining > 0}
                      className={`px-5 py-3 rounded-2xl text-white transition-all font-semibold w-full sm:w-auto ${loading || cooldownRemaining > 0 ? "opacity-50 pointer-events-none bg-slate-800/30 border border-blue-500/10" : "bg-gradient-to-r from-blue-500 to-cyan-500 hover:shadow-2xl hover:shadow-blue-500/40 hover:scale-105 active:scale-95"}`}
                    >
                      Scan Now
                    </button>
                  )}
