...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const [imgRes, chapterRes] = await Promise.all([
      axios.get(`${BASE_URL}/at-home/server/${chapterId}`),
    ...
    if (mangaId && currentChapter) {
      const listRes = await axios.get(`${BASE_URL}/chapter?manga=${mangaId}&order[chapter]=asc&limit=500`)
