...
export default function LandingPage() {
  ...
  return (
    <>
      ...
      <main className="relative min-h-screen bg-gradient-to-b from-black via-[#0a0a1a] to-[#02010a] text-white overflow-hidden scroll-smooth">
        ...
        {/* Hero Section */}
        <section className="relative flex flex-col items-center justify-center text-center px-4 pt-20 pb-10 md:pt-28 md:pb-12">
          <motion.div
          ...
          >
            <Image
              ...
              width={160}
              height={160}
              className="w-32 sm:w-40 mx-auto rounded-full border-4 border-white shadow-[0_0_25px_rgba(0,200,255,0.7)] hover:scale-110 transition-transform duration-300"
            />
            ...
            {/* Language Toggle */}
            <div className="relative flex w-36 sm:w-40 h-9 bg-white/10 rounded-full overflow-hidden border border-white/20 mx-auto shadow-md">
              <motion.div
                layout
                className={`absolute top-1 left-1 w-1/2 h-7 rounded-full bg-gradient-to-r from-pink-500 to-blue-500 transition-transform duration-300 ${
                  lang === 'EN' ? 'translate-x-full' : ''
                }`}
              />
              <button
                onClick={() => setLang('ID')}
                className={`w-1/2 z-10 text-sm font-semibold ${
                  lang === 'ID' ? 'text-white' : 'text-gray-400'
                }`}
              >
                ID
              </button>
            ...
            <AnimatePresence mode="wait">
              <motion.p
                ...
                className="text-base sm:text-lg text-gray-300 max-w-xl mx-auto leading-relaxed px-2"
              >
        ...
        <section className="relative w-full max-w-7xl mx-auto mt-12 px-4 z-10">
          <h2 className="text-3xl font-bold mb-6 text-center text-white drop-shadow">Latest Anime Trending</h2>
          <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 sm:gap-5">
            {news.map((anime, index) => (
              <motion.div
                ...
                className="overflow-hidden rounded-xl border border-white/10 hover:border-blue-400 bg-white/5 backdrop-blur-md shadow-md hover:shadow-blue-400/20 transition-all duration-300 group"
              >
                <Link href={`/anime/${anime.id}`} className="block">
                  ...
                  />
                  <div className="bg-black/70 text-xs p-2 text-center truncate group-hover:whitespace-normal group-hover:text-sm transition-all duration-300">
        ...
        {/* Footer */}
        <footer className="relative mt-12 px-4">
