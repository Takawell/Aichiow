        ...
        {anime.slice(0, 10).map((item) => (
          ...
          >
            <div className="overflow-hidden rounded-xl bg-neutral-900 border border-neutral-800 shadow-sm group-hover:shadow-md group-hover:border-indigo-500/40 transition-all duration-300">
              ...
              {/* TEXT */}
              <div className="relative z-20 p-3 h-[80px] flex flex-col justify-between">
                <h3 className="text-sm font-semibold text-white group-hover:text-indigo-400 transition-colors duration-300 line-clamp-2 leading-snug">
                ...
                <div className="flex gap-1 mt-1 overflow-hidden">
                  {item.genres?.slice(0, 2).map((genre) => (
                    <span
                      key={genre}
                      className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/70 max-w-[70px] truncate"
                    >
                      {genre}
                    </span>
                  ))}
